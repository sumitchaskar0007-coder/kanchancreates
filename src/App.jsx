  ...
  return (
    <div className="site-shell">
      ...
      <main>
        ...
        <section id="contact" className="contact section-pad"><div className="contact-intro"><p className="eyebrow light"><span /> Start a conversation</p><h2>Have a space<br /><em>in mind?</em></h2><p>Tell us a little about your project. We’ll bring the right people to the table.</p><div className="contact-details"><img className="contact-logo" src={kanchanLogo} alt="Kanchan Architect and Interior Designers" /><span>Munot Chambers, Navi Peth, opposite Titan Showroom<br />Kapad Bazar, Ahmednagar 414001, Maharashtra</span></div></div><form className="contact-form" action={formAction}><label>Name<input name="name" type="text" placeholder="Your name" required /></label><label>Email<input name="email" type="email" placeholder="you@company.com" required /></label><label>Phone<input name="phone" type="tel" placeholder="+91" /></label><label>Tell us about the project<textarea name="message" rows="3" placeholder="A few words about your requirement..." /></label><button className="button button-light" type="submit" disabled={isPending}>{isPending ? 'Sending...' : 'Send enquiry ↗'}</button>{formState.error && <p className="form-message error">{formState.error}</p>}{formState.success && <p className="form-message">{formState.success}</p>}</form></section>
      </main>
