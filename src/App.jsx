...
const services = [
  ['01', 'Architecture', 'Thoughtful spaces shaped around the way people live, work and gather.'],
  ['02', 'Interior Design', 'Layered interiors that balance material, light, function and personality.'],
  ['03', 'Space Planning', 'Clear, practical layouts that make every square foot feel intentional.'],
  ['04', 'Turnkey Execution', 'From first sketch to final detail, one trusted team carrying the vision through.'],
  ['05', 'Renovation & Styling', 'Fresh perspective for existing spaces, with detail-led upgrades that last.'],
]
  ...
  return (
    <div className="site-shell">
      ...
      <main>
        ...
        <section className="stats"><div className="stat-lead">The details<br /><em>behind the feeling.</em></div><div className="stat"><strong>12<span>+</span></strong><small>Years of practice</small></div><div className="stat"><strong>80</strong><small>Spaces shaped</small></div><div className="stat"><strong>100<span>%</span></strong><small>Personal attention</small></div></section>
        ...
        <section className="projects section-pad section-dark"><div className="section-heading compact"><div><p className="eyebrow light"><span /> Selected work</p><h2>Made to be<br /><em>noticed.</em></h2></div><p className="section-intro">A glimpse at the places, structures and identities we help bring to life.</p></div><div className="project-grid">{projects.map(([number, title, type, image], index) => <article className={`project project-${index + 1}`} key={title}><div className="project-art"><img src={image} alt={`${title} petrol retail project`} /><span className="project-number">{number}</span><span className="project-shape" /></div><div className="project-meta"><div><small>{type}</small><h3>{title}</h3></div><span>↗</span></div></article>)}</div></section>

        <section id="about" className="about section-pad"><div className="about-aside"><p className="eyebrow"><span /> Who we are</p><div className="about-stamp">K<br /><small>EST. 2012</small></div></div><div className="about-copy"><h2>A point of view<br /><em>you can feel.</em></h2><p>Kanchan Architect and Interior Designers creates spaces with a quiet confidence. We believe the best rooms are not just beautiful — they feel natural, personal and entirely yours.</p><div className="about-columns"><div><small>Our approach</small><p>Understand the people, respect the architecture, and make every detail count.</p></div><div><small>Our promise</small><p>Thoughtful design, honest guidance and a finished space you will love living in.</p></div></div></div></section>

        <section id="contact" className="contact section-pad"><div className="contact-intro"><p className="eyebrow light"><span /> Start a conversation</p><h2>Have a space<br /><em>in mind?</em></h2><p>Tell us a little about your project. We’ll bring the right people to the table.</p><div className="contact-details"><img className="contact-logo" src={kanchanLogo} alt="Kanchan Architect and Interior Designers" /><span>India / Residential &amp; commercial</span></div></div><form className="contact-form" action={formAction}><label>Name<input name="name" type="text" placeholder="Your name" required /></label><label>Email<input name="email" type="email" placeholder="you@company.com" required /></label><label>Phone<input name="phone" type="tel" placeholder="+91" /></label><label>Tell us about the project<textarea name="message" rows="3" placeholder="A few words about your requirement..." /></label><button className="button button-light" type="submit" disabled={isPending}>{isPending ? 'Sending...' : 'Send enquiry ↗'}</button>{formState.error && <p className="form-message error">{formState.error}</p>}{formState.success && <p className="form-message">{formState.success}</p>}</form></section>
      </main>
